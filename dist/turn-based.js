...
function addLog(snapshot, message) {
    return [message, ...snapshot.activeLog].slice(0, 6);
}
function initialSnapshot(campaign, seed) {
...
function rejected(snapshot, code, message) {
    return {
...
export class TurnBasedEngine {
    ...
    dispatch(command) {
        ...
        if (action.effect !== "guard") {
            ...
            if (before.level.enemy.behavior === "heavy")
                incoming += 2;
            if (before.level.enemy.behavior === "charger" && before.turn % 2 === 0)
                incoming += 4;
            if (before.level.enemy.behavior === "slow")
            ...
            if (before.level.enemy.behavior === "drainer")
            ...
            if (defeated)
                events.push({ type: "turn.defeat", message: "The encounter is lost. Rebuild and retry.", levelId: before.level.id });
        }
        else {
...
export function createTurnBasedEngine(config = {}) {
    return new TurnBasedEngine(config);
}
export function actionById(campaign, actionId) {
